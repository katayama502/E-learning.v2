...
export async function fetchAdminStats() {
    try {
        ...
        // 4. Fetch Learning Content Count
        const { count: learningCount, error: learningError } = await supabaseAdmin
            .from('course_lessons')
            .select('*', { count: 'exact', head: true });

        if (learningError) console.warn('Learning count error:', learningError);
...
export async function fetchQuestsAction() {
    try {
        ...
        // Fetch reels for these quests safely
        let allReels: any[] = [];
        let applyInteractions: any[] = [];

        if (data && data.length > 0) {
            const questIds = data.map(q => q.id);
            // Process in chunks to prevent URL length limits
            for (let i = 0; i < questIds.length; i += 100) {
                const chunk = questIds.slice(i, i + 100);
                
                // Fetch reels
                const { data: reelsChunk } = await supabaseAdmin
                    .from('media_library')
                    .select('*')
                    .in('job_id', chunk);
                if (reelsChunk) allReels = allReels.concat(reelsChunk);

                // Fetch application counts
                const { data: interactionsChunk } = await supabaseAdmin
                    .from('interactions')
                    .select('target_id')
                    .eq('type', 'apply')
                    .in('target_id', chunk);
                if (interactionsChunk) applyInteractions = applyInteractions.concat(interactionsChunk);
            }
        }
...
export async function fetchPublicJobsAction() {
    try {
        ...
        // Fetch reels for these jobs and companies safely in chunks
        const jobIds = data ? data.map(j => j.id) : [];
        const orgIds = data ? data.map(j => j.organization?.id).filter(Boolean) : [];
        let allReels: any[] = [];

        if (jobIds.length > 0 || orgIds.length > 0) {
            for (let i = 0; i < Math.max(jobIds.length, orgIds.length); i += 100) {
                const jChunk = jobIds.slice(i, i + 100);
                const oChunk = orgIds.slice(i, i + 100);
                
                let query = supabaseAdmin.from('media_library').select('*');
                if (jChunk.length > 0 && oChunk.length > 0) {
                    query = query.or(`job_id.in.(${jChunk.join(',')}),organization_id.in.(${oChunk.join(',')})`);
                } else if (jChunk.length > 0) {
                    query = query.in('job_id', jChunk);
                } else if (oChunk.length > 0) {
                    query = query.in('organization_id', oChunk);
                }
                
                const { data: reelsChunk } = await query;
                if (reelsChunk) allReels = allReels.concat(reelsChunk);
            }
        }
